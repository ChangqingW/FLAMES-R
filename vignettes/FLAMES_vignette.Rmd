...
Once the initial variables have been setup, the pipeline can created using the `BulkPipeline()`, `SingleCellPipeline()` or the `MultiSampleSCPipeline()` constructors. We will use `SingleCellPipeline` as we have a single scRNA-seq sample.
```{r, eval=TRUE, echo=TRUE}
library(FLAMES)
pipeline <- SingleCellPipeline(
  config_file = config_file, outdir = outdir, fastq = fastq, annotation = annot,
  genome_fa = genome_fa, expect_cell_number = 10
)
# run the pipeline
...
If coupled short-reads were available, we could use barcodes identified from the short-read data instead of using the long-read data to identify the cell barcodes. For example, the `filtered_feature_bc_matrix/barcodes.tsv.gz` from `cellranger` outputs can be used to create an allow-list, with `zcat barcodes.tsv.gz | cut -f1 -d'-' > allow-list.csv`. This file can then be provided as the `barcodes_file` argument in the `SingleCellPipeline()` and `MultiSampleSCPipeline()` constructors.

The pipeline can alse be run by calling the consituent steps sequentially:
```{r, eval=TRUE, echo=TRUE}
# get a new pipeline instance in a new directory
pipeline2 <- SingleCellPipeline(
  config_file = config_file, outdir = tempfile(),
  fastq = fastq, annotation = annot,
  genome_fa = genome_fa, expect_cell_number = 10
)
pipeline2 <- run_step(pipeline2, "barcode_demultiplex")
pipeline2 <- run_step(pipeline2, "genome_alignment")
```

The pipeline can be resumed (from incomplete manual runs or errors) with `resume_FLAMES(pipeline)`, which will run all configured but not yet completed steps:
```{r, eval=TRUE, echo=TRUE}
